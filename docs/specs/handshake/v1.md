...
当 Catalog 同时声明 `streaming.enabled = true` 时，`object_sync_stream`
可以使用 NDJSON：每行一个 `CommercialObject`。Catalog 应把流拆成有界 chunk
提交，并使用 `batch_id + chunk ordinal + request_hash` 支持 provider 在传输中断后
重试同一条 stream。重试必须复用同一个 `batch_id` 和同一个 chunking 参数；
已提交 chunk 会按 `request_hash` replay。改变 chunk 边界属于新的写入请求，
已提交 chunk 会按 hash conflict 拒绝。

stream 的 `batch_id` 同时作为 `sync_run_id`。支持生产级 ingestion 的 Catalog
应暴露 run status endpoint，让 provider 能查询已提交 checkpoint、计数和 terminal
状态后再重试。Provider 调用 run status/complete endpoint 时必须传入 `provider_id`，
因为 `sync_run_id` 是 provider-scoped identity。Catalog 在提交 object fact、chunk
receipt 和 run checkpoint 的同一事务中，应写入 durable side-effect intent，例如
search index job 和 activity event；worker 崩溃后 stale claimed outbox work 必须可重新 claim。
...
只有支持 provider-to-catalog ingestion 的 Catalog 才需要在
`CatalogManifest.endpoints.object_sync` 中声明该 endpoint。实时转发型、
纯 affiliate、或只提供 query/resolve 的 Catalog 可以省略它。完整同步 payload
由运行时 schema 维护。

### 13.7 POST /ocp/objects/sync/stream

用于 NDJSON 流式对象同步。

只有同时声明支持 streaming sync capability 的 Catalog 才需要在
`CatalogManifest.endpoints.object_sync_stream` 中声明该 endpoint。返回消息体为
`ObjectSyncStreamResult`，由 `packages/ocp-schema` 维护。

### 13.8 GET /ocp/object-sync-runs/{sync_run_id}?provider_id={provider_id}

用于查询对象同步 run 的 checkpoint、计数、状态和 terminal result summary。

只有支持 provider-to-catalog ingestion 且持久化 run ledger 的 Catalog 才需要在
`CatalogManifest.endpoints.object_sync_run` 中声明该 endpoint。`provider_id` 是必填 query，
用于消除不同 provider 复用同一 `sync_run_id` 时的歧义。Provider 可在网络中断、客户端重启
或收到 5xx 后查询该 endpoint，再决定重试完整 stream 或从后续 chunk 恢复。

### 13.9 POST /ocp/object-sync-runs/{sync_run_id}/complete?provider_id={provider_id}

用于显式完成已提交全部 chunk 的 run。当前 commerce catalog 的 NDJSON stream 正常读到
EOF 时会由 Catalog 自动 complete；该 endpoint 主要用于 provider 或管理面在异步导入场景中
提交 terminal intent。`provider_id` 同样是必填 query。

### 13.10 与运行时 schema 的关系

当前仓库中，完整的运行时 payload 仍由 `packages/ocp-schema` 维护，包括：

- `ObjectSyncRequest` / `ObjectSyncResult` / `ObjectSyncStreamResult` / `ObjectSyncRun`
