...
- Treat `docs/ocp_catalog_center_protocol_v1.md` as the source of truth for the OCP Center / Catalog Registry handshake.
...
- Do not call a single Catalog Node implementation an OCP Center. A Catalog Node indexes commercial objects. An OCP Center indexes Catalog nodes.
...
- Center-level search returns Catalog profiles and route hints. Catalog-level search returns CommercialObject/CatalogEntry results.
- OCP Center must not default to syncing full object databases from registered Catalogs. It should index profiles, contracts, query capabilities, trust metadata, and route hints first.
...
- OCP Center persistence work should target `registered_catalogs`, `catalog_registration_records`, `catalog_manifest_snapshots`, `catalog_index_entries`, `catalog_verification_records`, `catalog_health_checks`, and `catalog_search_audit_records`.
