...
export async function verifyChallenge(challenge: StoredChallenge, expectedCatalogId: string, expectedRegistrationId: string) {
  ...
  if (challenge.challenge_type === 'https_well_known') {
    return verifyHttpsWellKnownChallenge(challenge, expectedCatalogId, expectedRegistrationId);
  }
...
async function verifyHttpsWellKnownChallenge(
  ...
  const matches = payload.registration_id === expectedRegistrationId &&
