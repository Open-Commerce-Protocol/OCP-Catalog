...
export const registeredCatalogs = pgTable('registered_catalogs', {
  ...
  centerCatalogUnique: uniqueIndex('registered_catalogs_center_catalog_unique').on(table.centerId, table.catalogId),
  statusIdx: index('registered_catalogs_status_idx').on(table.centerId, table.status, table.verificationStatus),
}));
...
export const catalogRegistrationRecords = pgTable('catalog_registration_records', {
  ...
  catalogRegistrationIdx: index('catalog_registration_records_catalog_idx').on(table.centerId, table.catalogId),
}));
...
export const catalogManifestSnapshots = pgTable('catalog_manifest_snapshots', {
  ...
  catalogSnapshotIdx: index('catalog_manifest_snapshots_catalog_idx').on(table.centerId, table.catalogId, table.createdAt),
}));
...
export const catalogIndexEntries = pgTable('catalog_index_entries', {
  ...
  centerCatalogIndexUnique: uniqueIndex('catalog_index_entries_center_catalog_unique').on(table.centerId, table.catalogId),
  catalogIndexStatusIdx: index('catalog_index_entries_status_idx').on(table.centerId, table.entryStatus, table.verificationStatus),
}));
...
export const catalogVerificationRecords = pgTable('catalog_verification_records', {
  ...
  catalogVerificationIdx: index('catalog_verification_records_catalog_idx').on(table.centerId, table.catalogId),
}));
...
export const catalogHealthChecks = pgTable('catalog_health_checks', {
  ...
  catalogHealthIdx: index('catalog_health_checks_catalog_idx').on(table.centerId, table.catalogId, table.checkedAt),
}));
...
export const catalogSearchAuditRecords = pgTable('catalog_search_audit_records', {
  ...
  centerSearchAuditIdx: index('catalog_search_audit_records_center_created_idx').on(table.centerId, table.createdAt),
}));
