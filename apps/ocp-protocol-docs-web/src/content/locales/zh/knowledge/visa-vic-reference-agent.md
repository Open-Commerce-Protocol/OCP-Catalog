...
- 不是商家系统的代理人——商家的订单、税费、履约、退款仍由 [WooCommerce 集成概览](/knowledge/woocommerce-overview) 中描述的 Merchant Layer 承担。
...
这条链路严格遵守 [Search / Resolve / Action 三步法](/knowledge/search-resolve-action)：Search 给候选；Resolve 给 ActionBinding 与受限上下文；Action（这里是 checkout）只在用户确认之后才会发生。
