...
export function createAdminRoutes(deps: AdminDeps) {
  return new Elysia({ prefix: '/admin' })
    .get('/status', async ({ headers, set }) => {
      if (!isAuthorized(headers, deps.cfg.WC_PROVIDER_ADMIN_KEY)) return unauthorized(set);
      ...
      return {
        ...
        site_url: deps.cfg.WC_PROVIDER_SITE_URL ?? null,
        api_version: deps.cfg.WC_PROVIDER_API_VERSION,
        state: snapshot,
      };
    })
