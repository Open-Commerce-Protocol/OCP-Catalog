...
const configSchema = z.object({
  ...
  /** 保护 /admin/* 管理端点的 header token */
  OCP_PROVIDER_ADMIN_KEY: z.string().min(8),

  /** 保护 provider-owned resolve endpoint 的 header token */
  OCP_PROVIDER_HOOK_SECRET: z.string().min(8),
...
export function loadAlimamaConfig(env: NodeJS.ProcessEnv = process.env): AlimamaConfig {
  ...
  if (parsed.OCP_PROVIDER_ADMIN_KEY === parsed.OCP_API_KEY) {
    throw new Error('OCP_PROVIDER_ADMIN_KEY must be distinct from OCP_API_KEY');
  }
  if (parsed.OCP_PROVIDER_HOOK_SECRET === parsed.OCP_API_KEY) {
    throw new Error('OCP_PROVIDER_HOOK_SECRET must be distinct from OCP_API_KEY');
  }
