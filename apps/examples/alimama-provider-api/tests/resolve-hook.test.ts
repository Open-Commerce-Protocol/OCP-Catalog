 ...
 *
 * 关键验证:
 *   - 正常路径:hook 调 alimama → 返 ActionBinding[]
...
async function postResolveHook(app: ReturnType<typeof createResolveHookRoutes>, body: unknown) {
  return app.handle(
...
describe('resolve-hook endpoint', () => {
  test('alimama 正常返回 → 输出 ActionBinding[]', async () => {
    ...
    const body = (await res.json()) as { action_bindings: any[] };
    expect(body.action_bindings).toHaveLength(1);
    expect(body.action_bindings[0]).toMatchObject({
  ...
  test('alimama 抛 AlimamaApiError → 降级返空 binding', async () => {
    const client = fakeClient({
  ...
  test('alimama 返畸形响应 (data undefined) → 空 binding', async () => {
    const client = fakeClient({
      generatePrivilegeLink: async () =>
  ...
  test('真实 mock client(走 fixture)端到端', async () => {
    ...
    expect(body.action_bindings).toHaveLength(2);
    expect(body.action_bindings[0]!.url).toContain('s.click.taobao.com');
  });
