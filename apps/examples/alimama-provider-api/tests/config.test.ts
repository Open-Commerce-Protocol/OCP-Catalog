...
describe('loadAlimamaConfig', () => {
  ...
  test('provider admin/hook secret 不能复用 catalog API key', () => {
    expect(() =>
      loadAlimamaConfig({ ...validBase, OCP_PROVIDER_ADMIN_KEY: 'dev-api-key' } as any),
    ).toThrow(/OCP_PROVIDER_ADMIN_KEY/);
    expect(() =>
      loadAlimamaConfig({ ...validBase, OCP_PROVIDER_HOOK_SECRET: 'dev-api-key' } as any),
    ).toThrow(/OCP_PROVIDER_HOOK_SECRET/);
  });
