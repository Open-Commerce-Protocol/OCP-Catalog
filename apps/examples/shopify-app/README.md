...
it. On install it runs OAuth, stores the shop's offline access token in
Postgres, subscribes to product + lifecycle webhooks, registers the shop as an
OCP Provider, and syncs the shop's products into the OCP Catalog.
...
| `GET /auth/callback` | Verify HMAC + state, exchange code→token, persist, subscribe webhooks, register + first sync |
| `GET /app?shop=…` | Embedded landing (App Bridge bootstrap; minimal here) |
| `POST /webhooks/products` | products/{create,update,delete} → incremental sync (HMAC, retryable) |
| `POST /webhooks/app/uninstalled` | purge token + mark uninstalled |
| `POST /webhooks/compliance/{customers-data-request,customers-redact,shop-redact}` | mandatory GDPR webhooks |
...
(ngrok/cloudflared), and `SHOPIFY_APP_MOCK=false`. See the repo design doc
...
```bash
bun run --cwd apps/examples/shopify-app test    # unit: hmac, webhook, mapper
