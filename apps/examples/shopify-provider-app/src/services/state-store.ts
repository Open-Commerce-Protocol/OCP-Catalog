/**
 * Small disk-backed key/value store. Holds the active registration version
 * and the last_synced_at cursor between sync runs. Production deployments
 * should swap this for Redis/Postgres; the reference apps use JSON files so
 * they boot without external infra.
 */
...
export class StateStore {
  ...
  async load(): Promise<ProviderState> {
    ...
    if (this.path && existsSync(this.path)) {
      ...
      try {
        this.state = { ...INITIAL, ...(JSON.parse(text) as Partial<ProviderState>) };
      } catch {
