/**
 * Thin client for the OCP Catalog's provider-facing endpoints.
 * Mirrors apps/examples/commerce-provider-api/src/catalog-client.ts in shape.
 */
...
export class CatalogClient {
  ...
  constructor(private readonly cfg: ShopifyProviderConfig) {
    this.baseUrl = cfg.SHOPIFY_PROVIDER_CATALOG_BASE_URL.replace(/\/$/, '');
  }

  registerProvider(registration: Record<string, unknown>) {
    return this.request<Record<string, unknown>>('/ocp/providers/register', {
      method: 'POST',
      body: registration,
    });
  }
  ...
  syncObjects(request: Record<string, unknown>) {
    return this.request<Record<string, unknown>>('/ocp/objects/sync', {
      method: 'POST',
      body: request,
      includeApiKey: true,
    });
  ...
  async getProvider(providerId: string): Promise<CatalogProviderState | null> {
    try {
      return await this.request<CatalogProviderState>(`/ocp/providers/${providerId}`, {
        method: 'GET',
      });
    } catch (err) {
  ...
  private async request<T>(path: string, opts: {
    ...
    if (opts.includeApiKey) headers['x-api-key'] = this.cfg.SHOPIFY_PROVIDER_CATALOG_API_KEY;
    ...
    if (!res.ok) {
      throw new CatalogClientError(
        res.status,
        `Catalog ${opts.method} ${path} → ${res.status} ${res.statusText}`,
        payload,
      );
    }
    return payload as T;
