 ...
 *
 * 提供 4 个运维工具:
 *   - POST /admin/probe-query   手动跑一次 PDD goods.search,落地 CommercialObject 形态
 ...
 *
 * 全部要求 x-admin-key 头与 cfg.PDD_CATALOG_ADMIN_KEY 一致。
  ...
  return new Elysia({ prefix: '/admin' })
    .post(
      ...
      async ({ body, headers, set }) => {
        if (!isAuthorized(headers, deps.cfg.PDD_CATALOG_ADMIN_KEY)) {
          return unauthorized(set);
        }
        const pageSize = body?.pageSize ?? deps.cfg.PDD_DEFAULT_PAGE_SIZE;
        const upstream = await deps.pdd.listGoods({
        ...
        const objects = goods.map((item) =>
          mapGoodsToCommercialObject(item, {
            sourceId: sourceId(),
            catalogBaseUrl: deps.cfg.PDD_CATALOG_PUBLIC_BASE_URL,
          }),
        );
        return {
          source_id: sourceId(),
    ...
    .post(
      ...
      async ({ body, headers, set }) => {
        ...
        return syncOrdersOnce(
          { pdd: deps.pdd, ledger: deps.ledger, cfg: deps.cfg },
          { lookbackHours },
        );
      },
      {
    ...
    .get('/ledger', ({ query, headers, set }) => {
      ...
      return {
        ...
        entries: deps.ledger.list({ limit }).map((e) => ({
          ...
          rawOrderStatus: e.rawOrderStatus,
          orderStatusDesc: e.orderStatusDesc,
