...
协议只强制要求 `endpoints.query` 和 `endpoints.resolve`。`provider_registration`、`object_sync`、`object_sync_stream`、`contracts`、`provider_contract` 只有在 Catalog 实现这些表面时才出现。实时分佣 Catalog 可以完全省略 Provider ingestion endpoints。
