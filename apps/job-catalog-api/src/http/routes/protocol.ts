...
import { buildCatalogManifest, buildWellKnownDiscovery } from '@ocp-catalog/catalog-core';
...
const DATA_PROFILE_CACHE_TTL_MS = 300_000;
const STREAM_SYNC_DEFAULT_CHUNK_SIZE = 500;
const STREAM_SYNC_MAX_CHUNK_SIZE = 1_000;

export function protocolRoutes(context: JobCatalogRuntimeContext) {
  const {
    config,
    db,
    activityEvents,
  ...
  let catalogDataProfileLoading: Promise<Awaited<ReturnType<typeof loadCatalogDataProfile>>> | null = null;

  return new Elysia()
    .get('/ocp/health', async () => getCatalogHealth())
    .get('/.well-known/ocp-catalog', () => buildWellKnownDiscovery(toCatalogCoreConfig(config) as any))
    .get('/ocp/manifest', async () => {
      const dataProfile = await getCatalogDataProfile();
      return buildCatalogManifest(toCatalogCoreConfig(config) as any, jobCatalogScenario, { dataProfile });
    })
    .get('/ocp/contracts', () => ({
      ocp_version: '1.0',
      kind: 'ObjectContractList',
      catalog_id: config.CATALOG_ID,
      contracts: buildCatalogManifest(toCatalogCoreConfig(config) as any, jobCatalogScenario).object_contracts,
    }))
    ...
    .post('/ocp/objects/sync/stream', async ({ request, query, headers }) => {
      ...
      return syncObjectStream(request, query);
    })
    ...
    .get('/ocp/providers/:providerId/objects', async ({ params, query }) => {
      const limit = optionalQueryInteger(query, 'limit') ?? 100;
      if (limit > 500) throw new AppError('validation_error', 'limit must be from 1 to 500', 400, { limit });
      const result = await services.objects.listProviderObjects(params.providerId, {
        limit,
    ...
    .post('/ocp/query', async ({ body, headers }) => {
      const result = await jobQueryService.query(body, {
    ...
    try {
      ...
      return {
        ...
        details: {
          ...
          semantic_search_enabled: config.JOB_CATALOG_SEMANTIC_QUERY_ENABLED,
        },
        dependencies: [
          {
            name: 'postgres',
  ...
  async function getCatalogDataProfile() {
    ...
    if (!catalogDataProfileLoading) {
      catalogDataProfileLoading = loadCatalogDataProfile().finally(() => {
        catalogDataProfileLoading = null;
      });
    }
    const value = await catalogDataProfileLoading;
    catalogDataProfileCache = { expiresAt: now + DATA_PROFILE_CACHE_TTL_MS, value };
    return value;
  }

  async function loadCatalogDataProfile() {
    const catalogEntryCount = await readActiveJobCount();
    return {
  ...
  async function readActiveJobCount() {
    const [row] = await db
      .select({ value: schema.catalogRuntimeStats.activeJobCount })
      .from(schema.catalogRuntimeStats)
      .where(eq(schema.catalogRuntimeStats.catalogId, config.CATALOG_ID))
      .limit(1);
    if (!row) {
      throw new AppError('internal_error', 'catalog_runtime_stats is not initialized', 503, {
        catalog_id: config.CATALOG_ID,
      });
    }
    return row.value;
  ...
  async function syncObjectStream(
    ...
    for await (const value of request.body) {
      ...
      pending = lines.pop() ?? '';
      for (const line of lines) await pushStreamLine(line);
    }
    pending += decoder.decode();
    if (pending.length > 0) await pushStreamLine(pending);
