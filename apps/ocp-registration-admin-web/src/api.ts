...
export type RegistrationCatalogListItem = {
  catalog_id: string;
  homepage: string;
...
export type RegistrationSearchAudit = {
  id: string;
  created_at: string;
  result_count: number;
...
const registrationBaseUrl = resolveApiBaseUrl(
  ...
  'http://localhost:4100',
  resolveRegistrationApiProdDefault(),
);
const registrationAdminBaseUrl = `${registrationBaseUrl}/api/registration-admin`;

export async function fetchRegistrationAdminOverview(apiKey: string) {
  return request<RegistrationOverview>(`${registrationAdminBaseUrl}/overview`, { method: 'GET', apiKey });
}

export async function fetchRegistrationAdminCatalogs(apiKey: string) {
  const payload = await request<{ registration_id: string; catalogs: RegistrationCatalogListItem[] }>(`${registrationAdminBaseUrl}/catalogs`, { method: 'GET', apiKey });
  return Array.isArray(payload.catalogs) ? payload.catalogs : [];
}

export async function fetchRegistrationAdminRegistrations(apiKey: string, catalogId: string) {
  const payload = await request<{ registration_id: string; catalog_id: string; registrations: RegistrationRecord[] }>(
    `${registrationAdminBaseUrl}/catalogs/${catalogId}/registrations`,
    { method: 'GET', apiKey },
  );
  return Array.isArray(payload.registrations) ? payload.registrations : [];
}

export async function fetchRegistrationAdminSearchAudits(apiKey: string) {
  const payload = await request<{ registration_id: string; audits: RegistrationSearchAudit[] }>(`${registrationAdminBaseUrl}/search-audits`, { method: 'GET', apiKey });
  return Array.isArray(payload.audits) ? payload.audits : [];
}

export async function fetchRegistrationCatalog(catalogId: string) {
  const payload = await request<Record<string, unknown>>(`${registrationBaseUrl}/ocp/catalogs/${catalogId}`, { method: 'GET' });
  return mapRegistrationCatalog(payload);
}

export async function fetchRegistrationHealth(catalogId: string) {
  const payload = await request<{ checks: Record<string, unknown>[] }>(`${registrationBaseUrl}/ocp/catalogs/${catalogId}/health`, { method: 'GET' });
  return Array.isArray(payload.checks) ? payload.checks.map(mapRegistrationHealthRecord) : [];
}

export async function fetchRegistrationVerification(catalogId: string) {
  const payload = await request<{ records: Record<string, unknown>[] }>(`${registrationBaseUrl}/ocp/catalogs/${catalogId}/verification`, { method: 'GET' });
  return Array.isArray(payload.records) ? payload.records.map(mapRegistrationVerificationRecord) : [];
}

export async function fetchRegistrationManifestSnapshot(catalogId: string) {
  const payload = await request<Record<string, unknown>>(`${registrationBaseUrl}/ocp/catalogs/${catalogId}/manifest-snapshot`, { method: 'GET' });
  return mapRegistrationManifestSnapshot(payload);
}

export async function verifyRegistrationCatalog(catalogId: string) {
  return request<Record<string, unknown>>(`${registrationBaseUrl}/ocp/catalogs/${catalogId}/verify`, {
    method: 'POST',
    body: {},
  });
}

export async function refreshRegistrationCatalog(catalogId: string, catalogToken?: string) {
  return request<Record<string, unknown>>(`${registrationBaseUrl}/ocp/catalogs/${catalogId}/refresh`, {
    method: 'POST',
    body: {},
    catalogToken,
  });
...
export async function rotateRegistrationCatalogToken(catalogId: string, catalogToken?: string) {
  return request<Record<string, unknown>>(`${registrationBaseUrl}/ocp/catalogs/${catalogId}/token/rotate`, {
    method: 'POST',
    body: {},
    catalogToken,
  });
