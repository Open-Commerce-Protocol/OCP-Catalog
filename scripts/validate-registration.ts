...
await check('Registration health', async () => {
  const health = await get(`${registrationBaseUrl}/health`);
  assert(health.ok === true, 'registration health should be ok');
});

await check('Registration discovery and manifest', async () => {
  const discovery = await get(`${registrationBaseUrl}/.well-known/ocp-registration`);
  assert(discovery.registration_id === registrationId, 'registration_id should match');
  const manifest = await get(discovery.manifest_url);
  assert(manifest.kind === 'RegistrationManifest', 'registration manifest kind should match');
});
...
await check('Catalog refresh requires token', async () => {
  const response = await fetch(`${registrationBaseUrl}/ocp/catalogs/${catalogId}/refresh`, {
    method: 'POST',
...
await check('Catalog token rotation invalidates old token', async () => {
  const rotated = await post(`${registrationBaseUrl}/ocp/catalogs/${catalogId}/token/rotate`, {}, {
    'x-catalog-token': catalogToken,
  });
  assert(rotated.catalog_access_token, 'rotation should return new token');
  const oldResponse = await fetch(`${registrationBaseUrl}/ocp/catalogs/${catalogId}/refresh`, {
    method: 'POST',
    headers: {
...
console.log(`\nRegistration validation passed (${checks.length} checks).`);
